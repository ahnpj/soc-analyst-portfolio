...
- To further investigate the nature of the attack, I examined the http_user_agent field in the Suricata logs. This field identifies the software or tool that generated each HTTP request, which helps determine whether the traffic originated from a legitimate browser or an automated scanner. Within this field, I found entries containing the string acunetix_wvs_security_test, a known signature used by the Acunetix web vulnerability scanner. Combined with the presence of the Shellshock (CVE-2014-6271) exploit pattern, this confirms that the attacker was using Acunetix to perform automated reconnaissance and vulnerability testing against the Joomla server.
- Summary:
